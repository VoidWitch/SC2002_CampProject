...
public class Student extends User {
    ...
    public void viewRemainingSlots() {
        for(Camp opCamp: openCamps) {
            System.out.println("\nCamp: "+opCamp.getCampName());
        	System.out.println("Remaining Participant Slots: " + opCamp.getRemainingParticipantSlots());
        	System.out.println("Remaining Committee Slots: " + opCamp.getRemainingCommitteeSlots());
        }   
    }
        ...
        switch(choice) {
            case 1:
                ...
                if(selectedCamp != null) {
                    if(checkIfRegistered(selectedCamp)) {
                    	 System.out.println("You have already registered the camp!");
                         break;
                    }
                    ...
                    System.out.println("Successfully registered for Camp '" + selectedCamp.getCampName() + "' as an attendee.");     
                }      
                else
            ...
            case 2:
                if(isCampCommitteeMember) {
                    System.out.println("\nOnly allowed to be in camp committee for one camp.");
                    return;
                }      
                int campNo = 1;
                int campOpt;
                ...
                if(chosenCamp != null) {
                    ...
                    System.out.println("Successfully registered for Camp '" + chosenCamp.getCampName() + "' as camp committee member.");
                }
    ...
    public void submitEnquiry(Camp camp) {
        ...
        System.out.println("Enter your enquiry for Camp '" + camp.getCampName() + "': ");
        String enquiry = scanner.nextLine();
        Enquiry newEnquiry = new Enquiry();
        newEnquiry.setCampEnquiry(camp);
        ...
        if(ownEnquiries.isEmpty()) {
            ...
            for(Enquiry enquiry: ownEnquiries) {
                if(!enquiry.getProcessed()) {
                    System.out.println("Camp: " + enquiry.getCampEnquiry().getCampName());
    ...
    public void editEnquiry() {
        ...
        else if(ownEnquiries.get(choice - 1).getProcessed()) {
        	System.out.println("This enquiry has been processed. No edit is allowed.");
        }
        else {
            System.out.println("Invalid input. Enquiry edit unsuccessful.");
        }
    }
    ...
    public List<Camp> getRegisteredCamps(){
    	return this.enrolledCamps;
    }
    public boolean checkIfRegistered(Camp checkCamp) {
    	for(Camp camp: enrolledCamps) {
    		if(camp==checkCamp) {
    			return true;
    		}
